...
/*
cf) 컴포넌트: 리액트의 구성 요소 (UI)

!Props(Properties): 컴포넌트의 속성
: Props로 데이터 전달
<ChildComponent name="홍기수" />
- 함수형 컴포넌에서 데이터 받는 곳: 매개변수 
  ...
  : 부모 컴포넌트부터 자식 컴포넌트로 데이터를 전달할 때 사용
...
function MultiProps(props: {name: string; colorProps: string}) {
  // props.name
  // props.colorProps
  // : 여러 대의 props가 전달되더라도 하나의 객체로 전달 받음
  // > 하나의 객체에 전달 받거나
...
// 3) 상태 변경이 필요하면 부모로부터 콜백 함수를 수행해야 함
