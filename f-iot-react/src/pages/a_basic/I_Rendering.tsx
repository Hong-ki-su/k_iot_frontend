...
//! 렌더링(Rendering)
// : 작성한 코드를 화면에 출력하는 기능
// - React가 코드를(JSC/TSX)를 실제 브라우저 화면(UI)로 바꾸는 과정

// cf) React는 데이터(state, props)가 변경되면 필요한 부분만 다시 렌더링
// >> 화면 전체가 아니라 변경된 컴포넌트만 효울적으로 갱신 (재렌더링)
...
function Item({ name, isPacked }: ItemType) {
  ...
  // - 삼향 연산자 : JSX 안에서 표현 가능, 한 줄로 간결
  // - 논리 연산자: 조건이 참인 경우에만 조건이 거짓인 경우에만 and
...
function I_Rendering() {
  //! 배열 렌더링 
  //: 리액트 시 map() 메서드를 사용
  // - 리액트를 return문 앞에서 JSX 요소들의 배열을 렌더링
  // 순회한 후 데이터 값이 있어야 element에 데이터 전달

  // cf) forEach()는 단순 반복 실행만 하고 리턴값이 없음
  const people = ['최동욱', '김채환', '오제욱', '박성준', '최현우'];
...
const businessPeople = peopleDescription.filter(person => person.job
  =='강사');
