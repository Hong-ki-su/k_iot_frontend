...
const student1 = new Student('이상은', 20);
student1.greet('최광섭');
...
//? 인터페이스 확장
// extends 키워드
// A 와 B가 같은 형식 (클래스 / 인터페이스)

// cf) A implements B 키워드
// : A 와 B가 다른 형식 (A: 클래스, B: 인터페이스)
...
// 1) interface
// - extends 키워드로 확장 가능 (A인터페이스가 extends B인터페이스)
...
//% 2) type alias
// - &(인터렉션, 교차 타입)으로 확장 가능 (A타입별칭 & B타입별칭)
// - 같은 이름으로 중복 선언 불가
type B = { a: string; }
// - 원시 타입, 유니언, 인터렉션, 튜플 등 모든 타입 표현 가능
// - implements 사용은 가능하지만 권장 하지 않음