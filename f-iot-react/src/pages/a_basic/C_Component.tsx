...
import cat from '@/assets/images/고양이이미지.jpg';

/*
! 컴포넌트
: 사용자 인터페이스(UI)를 구촉하는 기본 단위 요소
- 함수형 컴포넌트 사용

? 컴포넌트 사용 방법
1) 컴포넌트 내보내기
 > export default : 컴포넌트 변경 가능  {} 미사용
...
2) 컴포넌트 가져오기
 > 불러올 환경에서 경로를 지정
  ...
  > vite.config.ts 와 tsconfig.app.json 파일에 각각 경로 지정 필수

  ? 컴포넌트 특징
  : 파일명이 반드시! 대문자로 시작 (UpperCameCase 사용)
  EX) MainContainer, NavBar 등
  - JS/TS 일반 함수 형태와 구분
  > 일반 함수 (lowerCameCase 사용)
...
// Img 컴포넌트
export function Img() {
  //? TS 코드는 내에서 HTML 코드 작성 시
  // : () 소괄호 내에 작성

  //? HTML 코드 내에서 TS 코드 작성 시
  //: {} 중괄호 내에 작성
  
  //? 함수형 컴포넌트(TSX)는 return시 HTML을 반환
  return (
