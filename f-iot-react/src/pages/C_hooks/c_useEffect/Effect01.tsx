...
//: React 함수형 컴포넌트 에서 부수효과를 수행하기 위한 Hook 기능
// - 데이터 가져오기 컴포넌트 렌더링 시 특정 작업 수행 등

//! useEffect (부수 효과, side Effect)
// : 컴포넌트 주요기능(UI 렌더링 상태관리)외에 발생하는 직업
// EX) API 호출, 이벤트 리스너 등록 수동 DOM조작
...
function Effect01() {
  ...
  // name값을 의존성 배열로 가지고 useEffect
  // : 마운트 시 + name값 상태 변경
  useEffect(() => {
    console.log('name값을 의존성 배열로 가지고 useEffect');
  }, [name]);

  // 빈 배열로 의존성 배열로 가지는 useEffect
  // : 마운트 시에만 실행
  useEffect(() => {
    console.log('빈 배열로 의존성 배열로 가지는 useEffect');
  }, []);
  ...
  useEffect(() => {
    ...
    // return () => {
    // clearInterval(timer);
    // console.log('타이머가 정리됨 (컨포넌트가 사라짐)');
    // }
  }, []);
  ...
  return (
    <div>
      ...
      <br />
      <span>{name}</span>
      <button onClick={() => setName(name === '홍기수' ? '이도경' : '홍기수')}>
        이름 변경
      </button>
