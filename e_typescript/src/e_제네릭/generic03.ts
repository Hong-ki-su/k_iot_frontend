...
function UpdateUser(id: number, changes: Partial<User>) {
  ...
  // 2) age 만 있는 경우 : { 기존의name, 기존의age, 변경될age} >> changes의 age값으로 수정
  // 3) 둘다 있는 경우 : { 기존의name, 기존의age, 변경될name, 변경될age} >> 모든 changes로 수정
  // 4) 둘다 없는 경우 : 수정 X
...
// 인사부(모든 데이터 필요), 회계부(주소 일 필요 없음), 총무부(주소, 전화번호 일 필요 없음)
