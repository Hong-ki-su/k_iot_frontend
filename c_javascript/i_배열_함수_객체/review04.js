...
: 학생 성적이 주어진 값(한계점) 이상인 학생을 필터링 + 내림차순 정렬 반환
> filter(), sort()
...
class GradeManagement {
  ...
  getTopStudent(threshold) {
    ...
    //  .sort(); // 오름차순 정렬 - sort((a, b) => a.average - b.average)
      .sort((a, b) => b.average - a.average); // 내름차순 정렬
...
// 학생 추가 예제
gradeSystem.addStudent("홍기수", {Math: 90, English: 85, Science: 78});
gradeSystem.addStudent("최동욱", {Math: 80, English: 95, Science: 88});
gradeSystem.addStudent("김채환", {Math: 85, English: 100, Science: 65});
gradeSystem.addStudent("오제욱", {Math: 70, English: 80, Science: 87});
