/*
!프로그램 기능 정의
: jsonplaceholder의 데이터를 사용하여 posts 게시몰 데이터 가져오기
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const userId = 1; // 사용할 특성 사용자 ID를 정의
  ...
  searchInput.addEventListener('input', () => {
    ...
    if (!searchTerm) {
      displayPosts(allPosts);
      clearError(); // 검색어를 내용이 없으면 에러를 제거
      return;
    }

    //! 검색어가 있는 경우
    const filteredPosts = filteredPosts(allPosts, searchTerm);

    if (filteredPosts.length === 0) {
      // 검색어는 있지만 해당 검색어와 일치하는 게시몰이 없는 경우
      showError('일치하는 포스특가 없습니다');
    } else {
