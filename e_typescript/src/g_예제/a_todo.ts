...
% 요구사항 정리
1. Todo(할 일, 객체) 항복의 인터페이스 정의(TodoItem)
...
function addTodo(todos: TodoItem[], task: string): TodoItem[] {
  const newTodo: TodoItem = {
    // id값은 기존의 Todo 항목 중에서 가장 큰 id값에 1을 더해 새로운 ID생성 - 중복 방지

    //? Math.max(): 인자로 주어진 숫자 중에서 가장 큰 수를 반환
    ...
    {id: 1, task: 'b', completed: true},
    {id: 1, task: 'c', completed: false}
    }
...
function completedTodo(todos: TodoItem[], id: number) {
  ...
  const changeTodos = todos.map(todo => 
    // 순회되는 각 요소의 todo.id 매개변수로 전달하는 id값(수정하고자 하는 요소)
