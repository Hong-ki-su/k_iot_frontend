...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const resetButton = document.getElementById('completed-button');
 ...
 // const colors = ['black', 'red', 'yellow', 'pink', 'orange', 'blue'];
  ...
  function revealCardsTemporary() {
    ...
    // ? DOM요소에 속성 지정
    // DOM 요소.setAttribute(속성, 속성값);
    // DOM요소.속성명 = 속성값;

    //? bool 속성
    // 속성값이 true, false뿐 (기본값; false)
    //disabled 속성: 요소에 대한 컨트롤이 비활성롸를 지정
  ...
  function disabledCards() {
      ...
      //? 새로운 카드가 담길 수 있도록 초기화 (전역 변수 초기화)
      resetBoard();
  }

  //@ 7. 매치되지 않은 카드를 다시 뒤집는 함수 정의
  function unflipCards() {
      lockBoard = true; // 게임판 정글
