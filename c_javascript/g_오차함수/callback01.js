...
// JS의 함수 
function funcName() {} // 함수 선언식 - function 필수 / 함수명 필수
const funcName2 = function () {} // 2) 함수 표현식 - function 필수 / 함수명 선택
const funcName3 = function ()  {} // 3) 화살표 함수 - function 생략 / 함수명 선택
...
function funcType() {}
console.log(typeof funcType); // typeof 연산자: 데이터의 타입을 문자열로 변환
// >> function
...
function callback1(callbackFunc) { // print 함수가 인자로 전달되면 callbackFunc 매개변수명으로 사용됨
  for (let i = 0; i < 3; i++) {
    callbackFunc(i); // i의 값은 print 함수의 index 매개변수로 전달
  }
}
...
const even = evenNum => console.log(`${even} 값은 짝수입니다`);
const evenFunc = evenNum => console.log(`${oddNum} 값은 홀수입니다`);
