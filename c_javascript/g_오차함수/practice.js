...
// 1번 문제
// 다음 배열을 조건에 따라 작성하고 실행했을 때의 결과를 확인하시요

const scores = [75, 83, 95, 64, 100, 55, 43];

function passingScores(scores) {
  // 점수가 60점이 넘는 점수의 개수와 평균을 하나의 배열로 반환
  ...
  const passingAverrage = passing.reduce((acc, score) => acc + score, 0) / passingCount;
...
// +) 구조 분해 할당에서 최항의 배열 내 변수는 변수 선언과 같음!
...
const fileteredWords = (words, substring) => {
  // 1) 첫 번째 인자: 배열
  // 2) 두 번째 인자: 필터링할 단어
  ...
  // >> 배열 문자열 등에서 데이터 인자값이 포함되어있는지를 검사 
